...
const songSchema = z.object({
  ...
  platform: z.enum(["Spotify"]), // Music platform (only Spotify is supported)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!result.success) { // If validation fails, return a 400 Bad Request with details about what's wrong
      return NextResponse.json(
    ...
    catch (error: any) { // If the database operation fails, return a 500 Internal Server Error
      return NextResponse.json(
        {
  ...
  catch (error: any) { // If JSON parsing or any other operation fails, return a 500 Internal Server Error
    return NextResponse.json(
      {
